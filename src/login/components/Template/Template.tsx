 ...
 *
 * biome-ignore-all lint/complexity/useOptionalChain: upstream keycloakify-shadcn pattern, kept verbatim to ease future syncs
 * biome-ignore-all lint/a11y/noLabelWithoutControl: upstream pattern (label associated via layout, not htmlFor)
 * biome-ignore-all lint/a11y/useValidAnchor: upstream — anchor used as form submitter (try-another-way)
 * biome-ignore-all lint/security/noDangerouslySetInnerHtml: `kcSanitize` is the explicit sanitiser for KC server messages
 */
...
import faviconUrl from './../../assets/img/favicon.svg?url'
...
export function Template(props: {
	...
	// Override Keycloak's default favicon with the AISFlow brand mark.
	// The base Keycloak template injects its own <link rel="icon">
	// pointing at the server's stock asset; we replace (or add) ours
	// so the SSO login tab matches the rest of AISFlow. Mount-only —
	// `faviconUrl` is a module-scoped Vite asset URL, never changes.
	useEffect(() => {
